...
- The capability registry versions skills, MCP servers, plugins and tools; records immutable test, state and permission receipts; and blocks dispatch unless an exact enabled version has a current passing test and sufficient scoped permission. A task may invoke the generated `aos.bounded-echo-v1` adapter or the engine-owned, read-only `aos.staged-text-reader@1` MCP server. Both revalidate at execution time and retain redacted receipts. One local deterministic task-workspace writer is also shipped. It accepts no path or content from the plan, claims an exact approval for the upcoming attempt, revalidates capability and task state around the write, records only fingerprints publicly, and can restore or remove the prior bytes from an engine-private bounded journal. Generic writable and externally acting execution remain unavailable.
