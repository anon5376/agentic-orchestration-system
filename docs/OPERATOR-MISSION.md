...
- Claude Code is mounted through its existing `claude.ai` account session. OpenAI Responses is mounted separately through a named API-key environment variable with `store: false`, no tools or continuation, exact-model verification, and provider usage receipts. Mixed runs keep assignments explicit; Grok and generic HTTP remain typed configuration boundaries. Dispatch-bound sessions wrap opaque harness references in AOS-owned authorization scope.
...
- OpenAI Responses is now mounted as a bounded direct API-key worker. It accepts only the default or explicitly enabled OpenAI API host, refuses task-supplied transport, tools, delegation, fallback and session resume, verifies the exact response model, and normalizes provider token counters into runtime receipts. Initial plans, templates, settings and appended plans share the same harness and admission rules. Automated checks use an injected transport; live account evidence is still pending.
...
- Which provider-specific API or officially supported OAuth path should follow the bounded OpenAI adapter.
