...
- Task-level questions, dependency blocking and lead-planner proposals persist across restart. Lead planning uses verified ChatGPT-login Codex at `gpt-5.6-luna`/`max`/read-only, requires explicit request IDs and operator acceptance, and exposes the same lifecycle through HTTP and CLI.
...
- The capability registry versions skills, MCP servers, plugins and tools; records immutable test, state and permission receipts; and blocks dispatch unless an exact enabled version has a current passing test and sufficient scoped permission. A task may invoke the generated `aos.bounded-echo-v1` adapter or the engine-owned, read-only `aos.staged-text-reader@1` MCP server. Both revalidate at execution time and retain redacted receipts. A separate durable claim protocol now binds an approved task-workspace write to its capability, task attempt, input, isolation and rollback fingerprints; it provides cross-engine ownership, fencing, lease recovery, terminal receipts and idempotent rollback receipts. No writable adapter invokes that protocol yet, so generic writable and externally acting execution remains unavailable.
...
- Policy proposals now require an immutable, comparable evaluation receipt before approval. Promotion records a versioned policy genome and rollback appends a restoring version. Evaluation metrics and artifact references are operator-attested; an AOS-run benchmark executor is not shipped yet.
...
3. **Planner contract — shipped for Codex.** A verified Luna lead planner proposes a bounded schema-valid plan. Ambiguity waits for operator answers, and no task becomes runnable until the operator accepts the active proposal.
...
5. **Capability registry — two bounded adapters and an effect-claim kernel shipped.** Skills, MCP servers, plugins and generated tools are version-pinned, tested, permissioned and revocable before assignment. Dispatch persists the resolved mount receipt. The generated `aos.bounded-echo-v1` tool accepts bounded engine-derived input. The engine-owned `aos.staged-text-reader@1` MCP server reads one staged declared file over a fixed stdio protocol. Both honour cancellation and timeout, revalidate immediately before invocation, and keep raw content out of state and events. The effect-claim kernel adds exact prior approval, cross-engine fencing, lease recovery, durable terminal receipts and rollback receipts for task-workspace writes. Generic discovery and actual effectful adapters remain pending.
...
7. **Measured improvement — shipped as an evidence gate for policy proposals.** Evaluation receipts bind quality, cost, latency, verified-runtime rate and operator intervention to the current proposal, full baseline and genome head. Promotion and rollback append immutable genome versions. Automated benchmark execution remains pending.
...
The next backend acceptance scenario is a deterministic task-workspace write adapter that uses the shipped effect-claim kernel end to end: acquire the exact approved claim before mutation, emit an apply receipt, execute the predeclared rollback path, and prove crash recovery without enabling network, shell, or external actions.
